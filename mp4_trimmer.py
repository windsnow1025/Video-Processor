...
def get_video_duration(ffprobe_path, video_file):
    result = subprocess.run(
        [ffprobe_path, "-v", "quiet", "-print_format", "json", "-show_format", video_file],
        capture_output=True,
        text=True
    )
...
def trim_video_end(ffmpeg_path, ffprobe_path, video_file, output_file, seconds_to_remove=10):
    ...
    subprocess.run(
        [ffmpeg_path, "-i", video_file, "-t", str(new_duration), "-c", "copy", output_file]
    )
...
def trim_mp4_end(ffmpeg_path, ffprobe_path, seconds_to_remove=10):
    mp4_files = list(Path("input").glob("*.mp4"))
